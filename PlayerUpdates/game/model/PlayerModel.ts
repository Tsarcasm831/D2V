...
export class PlayerModel {
    ...
    sync(config: PlayerConfig) {
        ...
        // THUMB: 
        // With new geometry (Thumb at +X, Palm +Z):
        // Rotate X+ moves tip towards +Z (Into Palm).
        if (this.rightThumb) {
            const prox = this.rightThumb.children.find(c => c.name === 'proximal');
            if (prox) {
                 // Flexion (X axis): Positive = In to palm
                 const tX = isHolding ? 0.6 : 0.1;
                 prox.rotation.x = lerp(prox.rotation.x, tX, damp);
                 
                 // Opposition/Correction (Z/Y axis)
                 // Just stabilize the thumb slightly
                 const tZ = isHolding ? -0.2 : 0;
