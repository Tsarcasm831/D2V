...
export class HandBuilder {
    static create(materials: PlayerMaterials, isLeft: boolean, arrays: any) {
        ...
        // Thumb is at +X (Lateral side).
        // Pinky is at -X (Medial side).
        ...
        palm.position.y = -0.045; // Center palm below wrist
        palm.castShadow = true;
        hand.add(palm);

        // FINGERS
        // Order: Index -> Pinky
        // Right Hand: Index starts near Thumb (+X) and moves Left (-X).
        // Left Hand: Index starts near Thumb (-X) and moves Right (+X).
        
        const fLengths = [0.08, 0.09, 0.085, 0.065]; 
        const fWidth = 0.018;
        const fDepth = 0.02;
        
        // Start offset slightly inside the palm width
        const startX = 0.03 * sideMult; 
        const spacing = 0.022 * sideMult; // Move towards center/pinky

        for(let i=0; i<4; i++) {
            const fGroup = new THREE.Group();
            
            // Calc X position: Move AWAY from thumb
            // Right (1): 0.03 -> 0.008 -> -0.014 -> -0.036
            // Left (-1): -0.03 -> -0.008 -> 0.014 -> 0.036
            ...
            pGeo.translate(0, -pLen/2, 0); // Pivot at top
        ...
        // THUMB
        // Right Hand: Thumb at +X.
        // Left Hand: Thumb at -X.
        const thumbGroup = new THREE.Group();
        thumbGroup.position.set(0.05 * sideMult, -0.03, 0.01);
        
        // Thumb Orientation
        // Rotate Z to angle away from fingers (Spread)
        // Rotate Y to face mostly inward (Opposition)
        thumbGroup.rotation.z = 0.5 * sideMult; 
        thumbGroup.rotation.y = -0.5 * sideMult; // Turn towards Palm (+Z)
