...
export class ShirtBuilder {
    static build(parts, config) {
        ...
        const scaleUVs = (mesh, radius, height, isSphere = false) => {
        ...
        const applyPlanarUVs = (mesh, scale) => {
            const geo = mesh.geometry;
        ...
        if (config.bodyType === 'male' && parts.maleChest) {
            ...
            [-1, 1].forEach(side => {
                ...
                const rAtYVal = rAtY; // Helper to avoid TS error
                const rx = rAtYVal;
                const rz = rAtYVal * torsoDepthScale;
                let nz = 0;
                if (Math.abs(nx) < rx) {
        ...
        armPairs.forEach(({ arm, forearm }) => {
            ...
            scaleUVs(sleeveGeo.parameters ? {geometry: sleeveGeo} : {geometry: sleeveGeo}, sleeveRadius, totalHeight); // Hacky pass
