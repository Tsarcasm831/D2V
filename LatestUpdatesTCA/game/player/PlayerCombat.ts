...
export class PlayerCombat {
    ...
    private static handleFishingInput(player: Player, dt: number, input: PlayerInput) {
        // Reset reclick block if mouse released
        if (!input.attack1) {
        ...
        // State 1: Already Fishing (Line Out) -> Hold to Reel
        if (player.isFishing) {
            if (input.attack1) {
        ...
        if (input.attack1) {
            ...
            // Oscillate Charge (0 to 1)
            player.fishingChargeTime += dt * 3.0; // Oscillation speed
            ...
            if (player.isChargingFishing) {
                ...
                // Skip the windup phase in animation since we held it
                // Windup is 0.3s. Start immediately at the flick.
                player.fishingTimer = 0.3; 
                
                // Set charge strength on the weapon userData for FishingAction to use
                const weapon = player.model.equippedMeshes.heldItem;
                if (weapon) {
                    // Map charge (0-1) to reasonable min/max strength
                    const minStr = 0.2; 
                    const str = minStr + player.fishingCharge * (1.0 - minStr);
                    weapon.userData.castStrength = str;
                }
                
                // Reset charge visual for next time
    ...
    private static updatePunchCombo(player: Player, dt: number, input: PlayerInput, obstacles: THREE.Object3D[]) {
        ...
        // Movement Impulses (Timings adjusted for 0.6s per punch)
        if (t > 0.2 && t < 0.4) {
